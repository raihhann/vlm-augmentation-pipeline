...
async def process_media_matrix(
    ...
    async def batch_event_generator():
        ...
        # Cache dictionary to store baseline inference results: (key) -> (original_output, t_orig)
        baseline_cache = {}
        ...
        if processing_mode == "excel":
            ...
            for idx, row in df.iterrows():
                ...
                # --- BASELINE INFERENCE (RUN ONCE PER ROW) ---
                cache_key = (media_path_disk, current_prompt)
                if cache_key not in baseline_cache:
                    yield (
                        f"data: Processing Baseline for [{idx + 1}/{total_rows}]: {img_name}...\n\n"
                    )
                    await asyncio.sleep(0.01)
        ...
        # ==========================================================
            ...
            for idx, uploaded_file in enumerate(v_files_pool):
                ...
                # --- BASELINE INFERENCE (RUN ONCE PER VIDEO) ---
                cache_key = (media_path_web, current_prompt)
                if cache_key not in baseline_cache:
                    yield (
